...
from cv_module.BiFormer.bra_legacy import BiLevelRoutingAttention

from cv_module.BiFormer._common import Attention, AttentionLePE, DWConv
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
        ...
        if before_attn_dwconv > 0:
            self.pos_embed = nn.Conv2d(
                dim, dim,  kernel_size=before_attn_dwconv, padding=1, groups=dim)
        else:
        ...
        self.mlp = nn.Sequential(nn.Linear(dim, int(mlp_ratio*dim)),
                                 DWConv(int(mlp_ratio*dim)
                                        ) if mlp_dwconv else nn.Identity(),
                                 nn.GELU(),
    ...
    def forward(self, x):
        ...
        if self.pre_norm:
            if self.use_layer_scale:
                ...
                x = x + self.drop_path(self.gamma2 *
                                       self.mlp(self.norm2(x)))  # (N, H, W, C)
            else:
                # (N, H, W, C)
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        stem = nn.Sequential(
            nn.Conv2d(
                in_chans, embed_dim[0] // 2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
            nn.BatchNorm2d(embed_dim[0] // 2),
            nn.GELU(),
            nn.Conv2d(embed_dim[0] // 2, embed_dim[0],
        ...
        dp_rates = [x.item()
                    for x in torch.linspace(0, drop_path_rate, sum(depth))]
        cur = 0
        for i in range(4):
            stage = nn.Sequential(
...
def biformer_tiny(pretrained=False, pretrained_cfg=None,
    ...
    if pretrained:
        ...
        checkpoint = torch.hub.load_state_dict_from_url(
            url=url, map_location="cpu", check_hash=True, file_name=f"{model_key}.pth")
