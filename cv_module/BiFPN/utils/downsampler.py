...
class Downsampler_Conv(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        padding = math.ceil(
            (stride * (out_size - 1) - in_size + dilation * (kernel_size - 1) + 1) / 2)
    ...
    def __init__(self,
        ...
        if mode == 'maxpool':
            ...
            if padding > kernel_size / 2:
                raise ValueError(
                    'pad should be smaller than half of kernel size in Pool2d')
            if stride < 2:
                raise ValueError('downsampling stride must be greater than 1')

            self.pool = nn.MaxPool2d(
                kernel_size, stride, padding, dilation, **kwargs)

        elif mode == 'avgpool':
            padding = math.ceil(
                (stride * (out_size - 1) - in_size + (kernel_size - 1) + 1) / 2)
            ...
            raise ValueError(
                'please select the mode between maxpool and avgpool')
