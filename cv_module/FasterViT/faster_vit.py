...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 2, 4, 3, 5, 1).reshape(-1,
...
def ct_dewindow(ct, W, H, window_size):
    bs = ct.shape[0]
    N = ct.shape[2]
    ct2 = ct.view(-1, W//window_size, H//window_size, window_size,
                  window_size, N).permute(0, 5, 1, 3, 2, 4)
...
def ct_window(ct, W, H, window_size):
    bs = ct.shape[0]
    N = ct.shape[2]
    ct = ct.view(bs, H // window_size, window_size,
                 W // window_size, window_size, N)
    ...
    def forward(self, input_tensor, local_window_size):
        ...
        if not self.grid_exists:
            ...
            if n_global_feature > 0 and self.ct_correct:
                ...
                for i in range(seq_length):
                    for j in range(seq_length):
                        ind = (i+1)*step_for_ct * \
                            self.window_size[0] + (j+1)*step_for_ct
                        indices.append(int(ind))

                top_part = relative_position_bias[:, indices, :]
                lefttop_part = relative_position_bias[:,
                                                      indices, :][:, :, indices]
            ...
            if n_global_feature > 0 and self.ct_correct:
                ...
                relative_position_bias[:, :n_global_feature,
                                       :n_global_feature] = lefttop_part
                relative_position_bias[:, :n_global_feature,
                                       n_global_feature:] = top_part
                relative_position_bias[:, n_global_feature:,
                                       :n_global_feature] = left_part
        ...
        if not self.grid_exists:
            ...
            if self.rank == 1:
                ...
                relative_coords_table = torch.stack(torch.meshgrid(
                    [relative_coords_h, relative_coords_w])).contiguous().unsqueeze(0)
                ...
                if not self.conv:
                    ...
                    self.pos_emb = self.cpb_mlp(
                        relative_coords_table.flatten(2))
                self.relative_bias = self.pos_emb
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.gamma3 = nn.Parameter(
            layer_scale * torch.ones(dim)) if use_layer_scale else 1
        self.gamma4 = nn.Parameter(
            layer_scale * torch.ones(dim)) if use_layer_scale else 1
        ...
        if sr_ratio > 1:
            ...
            self.hat_attn = WindowAttention(
                ...
                attn_drop=attn_drop, proj_drop=drop, resolution=int(
                    cr_tokens_total**0.5),
                seq_length=cr_tokens_total)

            self.hat_mlp = Mlp(
                in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
            self.hat_drop_path = DropPath(
                drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x, carrier_tokens):
        ...
        if self.sr_ratio > 1:
            ...
            ct = ct_dewindow(ct, self.cr_window*self.sr_ratio,
            ...
            ct = ct_window(ct, self.cr_window * self.sr_ratio,
                           self.cr_window * self.sr_ratio, self.cr_window)
        ...
        if self.sr_ratio > 1:
            ...
            ctr, x = x.split([x.shape[1] - self.window_size*self.window_size,
                             self.window_size*self.window_size], dim=1)
            ...
            if self.last and self.do_propagation:
                ...
                x = x + self.gamma1 * self.upsampler(ctr_image_space.to(
                    dtype=torch.float32)).flatten(2).transpose(1, 2).to(dtype=x.dtype)
        return x, ct
...
class TokenInitializer(nn.Module):
    ...
    def __init__(self,
        ...
        to_global_feature.add_module("pool", nn.AvgPool2d(
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        ct = x.view(B, C, H // self.window_size, self.window_size,
...
class FasterViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            in_chans=in_chans, in_dim=in_dim, dim=dim)
        ...
        for i in range(len(depths)):
            ...
            level = FasterViTLayer(dim=int(dim * 2 ** i),
                                   ...
                                   drop_path=dpr[sum(depths[:i]):sum(
                                       depths[:i + 1])],
                                   downsample=(i < 3),
                                   layer_scale=layer_scale,
...
if __name__ == "__main__":
    ...
    model = FasterViTLayer(256, 4, 20, 4, 4)
    out = model(torch.randn(1, 256, 20, 20))
    print(out.shape)
    model_test(model,
