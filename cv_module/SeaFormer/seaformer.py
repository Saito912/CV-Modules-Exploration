...
class SqueezeAxialPositionalEmbedding(nn.Module):
    def __init__(self, dim, shape):
        ...
        self.pos_embed = nn.Parameter(torch.randn(
            [1, dim, shape]), requires_grad=True)

    def forward(self, x):
        B, C, N = x.shape
        x = x + F.interpolate(self.pos_embed, size=(N),
...
class Sea_Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        qrow = self.pos_emb_rowq(
            q.mean(-1)).reshape(B, self.num_heads, -1, H).permute(0, 1, 3, 2)
        ...
        xx_row = self.proj_encode_row(xx_row.permute(
            0, 1, 3, 2).reshape(B, self.dh, H, 1))

        # squeeze column
        qcolumn = self.pos_emb_columnq(
            q.mean(-2)).reshape(B, self.num_heads, -1, W).permute(0, 1, 3, 2)
        kcolumn = self.pos_emb_columnk(
            k.mean(-2)).reshape(B, self.num_heads, -1, W)
        vcolumn = v.mean(-2).reshape(B, self.num_heads, -
                                     1, W).permute(0, 1, 3, 2)
...
class Sea_Attention_Block(nn.Module):
...
class BasicLayer(nn.Module):
    def __init__(self, block_num, embedding_dim, key_dim, num_heads,
        ...
        for i in range(self.block_num):
            self.transformer_blocks.append(Sea_Attention_Block(
...
class Fusion_block(nn.Module):
    def __init__(
        ...
        self.local_embedding = ConvModule(
            inp, embed_dim, kernel_size=1, norm_cfg=self.norm_cfg, act_cfg=None)
        self.global_act = ConvModule(
            oup, embed_dim, kernel_size=1, norm_cfg=self.norm_cfg, act_cfg=None)
        self.act = h_sigmoid()
...
class SeaFormer(nn.Module):
    def __init__(self,
        ...
        for i in range(len(cfgs)):
            smb = StackedMV2Block(
                cfgs=cfgs[i], stem=True if i == 0 else False, inp_channel=channels[i], norm_cfg=norm_cfg)
            setattr(self, f"smb{i + 1}", smb)

        for i in range(len(depths)):
            # stochastic depth decay rule
            dpr = [x.item()
                   for x in torch.linspace(0, drop_path_rate, depths[i])]
            trans = BasicLayer(
                block_num=depths[i],
...
if __name__ == "__main__":
    ...
    model = Sea_Attention_Block(256, 32, 4)
    inp = torch.randn(1, 256, 20, 20)
    outp = model(inp)
    print(outp.shape)
